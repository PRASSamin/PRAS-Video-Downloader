...
export const formatPageJson = (postHtml: CheerioAPI) => {
  ...
  const thumbnailElement = postHtml("meta[property='og:image']");
  const displayElement = postHtml("meta[property='og:video:secure_url']");
  const userName = postHtml("meta[property='og:title']").attr("content");

  if (videoElement.length === 0 || thumbnailElement.length === 0) {
    return null;
  }

  const videoUrl = videoElement.attr("content");
  const thumbnailUrl = thumbnailElement.attr("content");
  const displayUrl = displayElement.attr("content");
